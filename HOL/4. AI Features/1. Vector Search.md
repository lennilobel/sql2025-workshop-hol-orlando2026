...
For example, notice how the close together the vector values of "The Godfather" and "Crime Drama" are, indicating similarity. In contrast, the "Screwball Comedy" vector values are relatively distant from the vector values of the other two movies.
...
> **Note:** This configuration step is required only for SQL Server 2025. In Azure SQL Database, `sp_invoke_external_rest_endpoint` is enabled by default, so this step is unnecessary. Attempting to run `sp_configure` in Azure SQL Database will result in an error because that option is not supported.

Run the following T-SQL command to enable this feature in SQL Server 2025:
...
If everything is working properly, you should see the first few floating point values in the vector array returned by Azure OpenAI as a JSON array. Click on that, and SSMS will open a new tab to show you the full vector (all 1536 vector values).
...
▶ [Lab: Vector Utility Functions](https://github.com/lennilobel/sql2025-workshop-hol-orlando2026/blob/main/HOL/4.%20AI%20Features/2.%20Vector%20Utility%20Functions.md)
