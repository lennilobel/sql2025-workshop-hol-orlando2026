...
This lab builds upon the foundational concepts introduced in the previous vector search lab, this time applying those principles to a real-world relational database using AdventureWorks2025. Our goal is to move beyond primitive examples into more scalable and realistic scenarios by using vector search to find semantically similar products based on their names and descriptions.
...
## AdventureWorks2025 for AI

Let's enable the AdventureWorks2025 database for AI. This is a sample database that contains data about a fictional bicycle manufacturing company. It includes product data (including names and descriptions) that we wish to enable for vector search.
...
> **Note:** Because AdventureWorks2025 is a relatively small database, the performance benefits of DiskANN will not be readily apparent. The true power of DiskANN indexing shines when working with extremely large datasets (millions or billions of vectors), where it can significantly speed up queries while still maintaining high accuracy. Of course, the steps to implement DiskANN are the same regardless of dataset size, and the principles learned here apply equally to larger, more complex systems than AdventureWorks2025.

In SQL Server Management Studio (SSMS), right-click on the **AdventureWorks2025** database in Object Explorer and select **New Query** to open a new query window for this lab.
...
This query returns no results at all, as there are no products in the AdventureWorks2025 database that match this description. This demonstrates how the search can handle queries that do not match any existing products, returning an empty result set rather than irrelevant results. We are, after all, selling bikes, not apples.
...
Let's create a **DiskANN** index to enable fast ANN queries over the `ProductVector` column of the `Production.ProductVector` table. But first, DiskANN is still in preview, so we need to enable the preview features for the AdventureWorks2025 database. To do this, run the following command:
...
> **Important:** In the current release of SQL Server 2025, once you create a DiskANN vector index, the table becomes effectively read-only. To modify data, you must first drop the index, make your data changes, and then recreate the index. Azure SQL Database supports updates to tables with DiskANN vector indexes, but this capability is not yet available in SQL Server 2025.

Creating the index speeds up approximate nearest neighbor (ANN) queries—but it also prevents `INSERT`, `UPDATE`, and `DELETE` operations against the table. To demonstrate this, try inserting a row into the `Production.ProductVector` table after creating the index:

```sql
INSERT INTO Production.ProductVector VALUES (1, 1, NULL)
GO
```

This will fail with an error message indicating that data modifications are not permitted because the table has a DiskANN vector index. This is a current limitation of DiskANN vector indexing in SQL Server 2025 and does not apply to Azure SQL Database.
...
▶ [Lab: External AI Models](https://github.com/lennilobel/sql2025-workshop-hol-orlando2026/blob/main/HOL/4.%20AI%20Features/4.%20External%20AI%20Models.md)
