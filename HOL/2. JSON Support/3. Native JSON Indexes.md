...
> Presently, the optimizer does not automatically detect available JSON indexes. To ensure the index is utilized, we will provide an index hint in our queries using the `WITH (INDEX(...))` syntax to force its use. In the future, the optimizer will automatically leverage JSON indexes when appropriate.
...
Observe the execution plans for both queries; they should each show "JSON Index Seek" operations, indicating that the JSON index is being used effectively no matter what JSON property is queried.
...
▶ [Lab: JSON Path Expression Array Enhancements](https://github.com/lennilobel/sql2025-workshop-hol-orlando2026/blob/main/HOL/2.%20JSON%20Support/4.%20JSON%20Path%20Expression%20Array%20Enhancements.md)
