...
- There are two JSON object in the array (one for customer John Doe, and the other for customer Jane Smity), resulting in two rows being inserted into the `Customer` table.
...
Using SSMS 22, it's easy to view the JSON content in a formatted way. Click on the `CustomerJson` hyperlink on the first row in the results grid to open the JSON viewer tab for the first customer. This allows you to explore the JSON structure in an expandable and collapsible format, making it much easier to read and understand the nested data. Then close the JSON viewer tab to return to the results grid, and repeat the same process for the second row to view the second customer's JSON data.
...
Once again, click on the `CustomerJson` hyperlink in the results grid to view the before and after JSON content in the JSON viewer tab. Then, observe the difference between the two. The `preferred` property has been changed to `true`, and the `basket.status` has been updated to `DEAD`.
...
▶ [Lab: Native JSON Indexes](https://github.com/lennilobel/sql2025-workshop-hol-orlando2026/blob/main/HOL/2.%20JSON%20Support/3.%20Native%20JSON%20Indexes.md)
