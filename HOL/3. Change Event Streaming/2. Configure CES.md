...
Change Event Streaming is currently a preview feature, so we first need to enable the preview features for the new database. Then we'll be able to enable CES itself. To do this, run the following command:

```sql
ALTER DATABASE SCOPED CONFIGURATION SET PREVIEW_FEATURES = ON
```

Run the following query to verify that (at this stage) CES is not yet enabled on any databases:
...
▶ [Lab: Build the Event Hub Client](https://github.com/lennilobel/sql2025-workshop-hol-orlando2026/blob/main/HOL/3.%20Change%20Event%20Streaming/3.%20Build%20the%20Event%20Hub%20Client.md)
